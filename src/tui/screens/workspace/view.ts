...
export function view(model: WorkspaceScreenModel, width: number, height: number): string {
  ...
  const titleBar =
    chalk.hex(colors.primary).bold(icons.folder + title) +
  ...
  if (model.showCreateInput) {
    ...
    lines.push(
      chalk.hex(colors.textMuted)(keyHint('Enter', 'Create') + '  ' + keyHint('Esc', 'Cancel'))
    );
  } else if (model.showDeleteConfirm) {
    const warning =
      chalk.bgRed.white.bold(' DELETE WORKSPACE? ') +
      ' ' +
