import { AppModel, Cmd, Command, WorkspaceScreenModel, WorkspaceMsg } from '../../types.js';
import { workspaceStorage } from '../../../storage/workspaces.js';
export { view } from './view.js';

export function init(model?: AppModel): [WorkspaceScreenModel, Command] {
...
export function update(msg: WorkspaceMsg, model: WorkspaceScreenModel): [WorkspaceScreenModel, Command] {
    switch (msg.type) {
        ...
        case 'WORKSPACE_DOWN':
            return [{ ...model, cursor: Math.min(model.workspaces.length - 1, model.cursor + 1), error: null }, Cmd.none()];
        ...
        case 'WORKSPACE_CREATE_SUBMIT': {
            ...
            if (workspaceStorage.exists(name)) return [{ ...model, error: 'Workspace already exists' }, Cmd.none()];
        ...
        case 'WORKSPACE_DELETE': {
            ...
            if (toDelete === 'default') return [{ ...model, error: 'Cannot delete default workspace' }, Cmd.none()];
        ...
        case 'WORKSPACE_DELETE_CONFIRM': {
            const target = model.workspaces[model.cursor];
            if (target === 'default') return [{ ...model, error: 'Cannot delete default workspace' }, Cmd.none()];
